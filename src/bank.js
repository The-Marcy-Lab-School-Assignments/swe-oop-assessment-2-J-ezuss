class BankAccount {
  ...
  static #totalNumberOfAccounts = 0; // added static property

  constructor(accountNumber, ownerName) {
    this.accountNumber = accountNumber;
    this.ownerName = ownerName;

    BankAccount.#totalNumberOfAccounts++; // works after adding static property
  }

  deposit(amount) {
    this.#balance += amount; // forgot to invoke the private property # in this.#balance
  ...
  withdraw(amount) {
    if (amount > this.#balance) {
      //  needed the private method in this.#balance
      // needed private function
      console.log(`Withdrawal failed. Insufficient funds.`);
    } else {
      this.#balance -= amount; // needed the private method in this.#balance
      console.log(`Withdrew $${amount}. New balance: $${this.#balance}`);
    }
    return this.#balance; // needed the private method in this.#balance
  }
...
class Bank {
  ...
  addAccount(account) {
    this.accounts.push(account); // forgot to invoke the `this` method
  }

  getTotalBalance() {
    let total = 0;
    this.accounts.forEach((account) => {
      total += account.getBalance(); // forgot to write "getBalance()" instead of just balance
    });
    return total;
  }
